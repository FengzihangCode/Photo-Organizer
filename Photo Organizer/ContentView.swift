...
import SwiftUI
import Foundation
import Cocoa //未来可能用得上
import CoreML //通过Machine Learning的方式Sort photos automaticly by size
import AppKit //创建一个UIViewRepresentable包装器来将NSAttributedString显示在SwiftUI中
...
struct ContentView: View {
                        ...
                        VStack(alignment: .leading, spacing: 20) {
                            ...
                            HStack {
                                ...
                                VStack(alignment: .leading) {
                                    Text("Photo Organizer V1.6.0 Build 2")
                                    Text("作者: DannyFeng")
    ...
    private func organizePhotosByDate(sourceURL: URL, destinationURL: URL) {
        ...
        if loggingEnabled {
            logOperation(type: "Organize Photos", details: "Photos organized by date")
        }
    }
    ...
    private func organizePhotosByFileType(sourceURL: URL, destinationURL: URL) {
        ...
        // Log the operation if logging is enabled
        if loggingEnabled {
            logOperation(type: "Organize Photos", details: "Photos organized by file type")
        }
    }
    ...
    private func openOperationHistory() {
        // Logic to display the operation history 未实现
    }
